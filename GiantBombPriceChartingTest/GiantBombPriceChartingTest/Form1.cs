...
namespace GiantBombPriceChartingTest
{
    ...
    {
        const string PcApiKey = "APIKEYGOESHERE";
        const string GbApiKey = "APIKEYGOESHERE";
        PriceChartingApi pcClient = new PriceChartingApi(PcApiKey);
        GiantBomb.Api.GiantBombRestClient gbClient = new GiantBomb.Api.GiantBombRestClient(GbApiKey);

        public Form1()
        {
            InitializeComponent();
            if (PcApiKey == "APIKEYGOESHERE" || GbApiKey == "APIKEYGOESHERE")
            {
                throw new Exception("Don't forget your API Key");
            }
        }
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            List<Product> p = await pcClient.SearchProductsByQueryAsync(SearchBar.Text);
